...
export function AuthScreen(props?: { onAuthSuccess?: () => void; /** false داخل نافذة «إضافة حساب» حتى لا يستبدل الزائر الحساب الحالي */ allowGuestBrowse?: boolean }) {
    ...
    if (mode === "login") {
      ...
      if (!loginAwaitingOtp) {
        ...
        if (r.requiresOtp) {
          ...
          const reasonMsg =
            r.otpReason === "new_device"
              ? "جهاز جديد — "
              : r.otpReason === "two_factor"
                ? "المصادقة الثنائية مفعّلة — "
                : "";
          setInfo(
            `${reasonMsg}أُرسل كود التحقق إلى ${r.emailHint || "بريدك الإلكتروني"}. أدخل الـ 6 أرقام للمتابعة.`,
          );
          return;
        }
        loginFailCountRef.current = 0;
  ...
  return (
    <div className="relative z-10 flex min-h-[100dvh] flex-col items-center justify-center overflow-y-auto no-scrollbar bg-background px-6 pb-[max(1.5rem,var(--sab))] pt-[max(1rem,var(--sat))]">
